...
namespace UnityChan
{
    public class UTS2AudioGUI : ShaderGUI {
    ...
    // -----------------------------------------------------
        ...
        //AudioLink
        MaterialProperty use_Audiolink = null;
        MaterialProperty band_Mode = null;
        MaterialProperty audio_Band = null;
        MaterialProperty audio_Mask = null;
        MaterialProperty audio_Mix = null;
        MaterialProperty audio_Power = null;
        MaterialProperty audio_Bace_Power = null;
        //
        ...
        public void FindProperties(MaterialProperty[] props)
        {
            ...
            //AudioLink
            use_Audiolink = FindProperty("_Use_AudioLink", props);
            band_Mode = FindProperty("_Is_SingleBand", props);
            audio_Band = FindProperty("_AudioBand", props);
            audio_Mask = FindProperty("_AudioLink_Tex", props);
            audio_Mix = FindProperty("_AudioMix", props);
            audio_Power = FindProperty("_AudioEmiPower", props);
            audio_Bace_Power = FindProperty("_AudioEmiBasePower", props);
            //
    ...
    // --------------------------------
        ...
        private static class Styles
        {
            ...
            //AudioLink
            public static GUIContent audioemissiveTexText = new GUIContent("AudioMask","AudioMask : AudioLink用。SingleMode:Mask(R) MultiMode:Mask(RGBA)");
            //
    ...
    // --------------------------------
        ...
        void GUI_Emissive(Material material)
        {
            ...
            EditorGUILayout.Space();
            EditorGUILayout.BeginHorizontal();
            EditorGUILayout.PrefixLabel("AudioLink Emissive");
            //GUILayout.Space(60);
                if (material.GetFloat("_Use_AudioLink") == 0)
                {
                    if (GUILayout.Button("Off", shortButtonStyle))
                    {
                        material.SetFloat("_Use_AudioLink", 1);
                    }
                }
                else
                {
                    if (GUILayout.Button("Active", shortButtonStyle))
                    {
                        material.SetFloat("_Use_AudioLink", 0);
                    }
                }
            EditorGUILayout.EndHorizontal();

            if (material.GetFloat("_Use_AudioLink") == 1)
            {
                EditorGUI.indentLevel++;
                GUILayout.Label("    AudioLinkSettings", EditorStyles.boldLabel);

                m_MaterialEditor.FloatProperty(audio_Bace_Power, "AudioEmissionBasePower");
                m_MaterialEditor.FloatProperty(audio_Power, "AudioEmissionPower");

                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.PrefixLabel("AudioEmissionMode");
                //GUILayout.Space(60);
                if (material.GetFloat("_AudioMix") == 0)
                {
                    if (GUILayout.Button("Cut", shortButtonStyle))
                    {
                        material.SetFloat("_AudioMix", 1);
                    }
                }
                else
                {
                    if (GUILayout.Button("Fade", shortButtonStyle))
                    {
                        material.SetFloat("_AudioMix", 0);
                    }
                }
                EditorGUILayout.EndHorizontal();

                if (material.GetFloat("_Is_SingleBand") == 1)
                {
                    //Shaderプロパティ [KeywordEnum(Low, LowMid, HighMid, High)] をEumPopupで表現する.
                    //マテリアル内のアウトラインモードの設定を読み込み.
                    int _AudioBand_Setting = material.GetInt("_AudioBand");
                    //Enum形式に変換して、outlineMode変数に保持しておく.
                    if ((int)_AudioBand.Low == _AudioBand_Setting)
                    {
                        audioband = _AudioBand.Low;
                    }
                    else if ((int)_AudioBand.LowMid == _AudioBand_Setting)
                    {
                        audioband = _AudioBand.LowMid;
                    }
                    else if ((int)_AudioBand.HighMid == _AudioBand_Setting)
                    {
                        audioband = _AudioBand.HighMid;
                    }
                    else if ((int)_AudioBand.High == _AudioBand_Setting)
                    {
                        audioband = _AudioBand.High;
                    }

                    //EnumPopupでGUI記述.
                    audioband = (_AudioBand)EditorGUILayout.EnumPopup("AudioBand", audioband);
                    //値が変化したらマテリアルに書き込み.
                    if (audioband == _AudioBand.Low)
                    {
                        material.SetFloat("_AudioBand", 0);
                    }
                    else if (audioband == _AudioBand.LowMid)
                    {
                        material.SetFloat("_AudioBand", 1);
                    }
                    else if (audioband == _AudioBand.HighMid)
                    {
                        material.SetFloat("_AudioBand", 2);
                    }
                    else if (audioband == _AudioBand.High)
                    {
                        material.SetFloat("_AudioBand", 3);
                    }
                }

                GUILayout.Label("    AudioLinkMask", EditorStyles.boldLabel);
                m_MaterialEditor.TexturePropertySingleLine(Styles.audioemissiveTexText, audio_Mask);

                EditorGUI.indentLevel--;

                EditorGUILayout.BeginHorizontal();
                EditorGUILayout.PrefixLabel("MultiBandMode(Advanced)");
                //GUILayout.Space(60);
                if (material.GetFloat("_Is_SingleBand") == 0)
                {
                    if (GUILayout.Button("Active", shortButtonStyle))
                    {
                        material.SetFloat("_Is_SingleBand", 1);
                    }
                }
                else
                {
                    if (GUILayout.Button("Off", shortButtonStyle))
                    {
                        material.SetFloat("_Is_SingleBand", 0);
                    }
                }
                EditorGUILayout.EndHorizontal();
